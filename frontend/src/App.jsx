...
function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><div className="brand"><span className="brand-mark">✓</span> TaskFlow</div><nav>{[['dashboard', '▦', 'Dashboard'], ['tasks', '✓', 'Task list'], ['settings', '⚙', 'Settings']].map(([key, icon, label]) => <button key={key} className={section === key ? 'active' : ''} onClick={() => setSection(key)}><span>{icon}</span>{label}</button>)}</nav><p>Stay focused.<br />Make progress.</p></aside>
    <div className="app-main"><header><div><span className="header-label">WORKSPACE</span><h2>{titles[section]}</h2></div><details className="user-menu"><summary><span className="avatar">{user.username[0].toUpperCase()}</span><span className="user-name">{user.username}</span><span>⌄</span></summary><div className="user-dropdown"><div><strong>{user.username}</strong><small>{user.email || 'TaskFlow member'}</small></div><button onClick={() => setSection('settings')}>Account settings</button><button className="logout" onClick={logout}>Sign out</button></div></details></header>
