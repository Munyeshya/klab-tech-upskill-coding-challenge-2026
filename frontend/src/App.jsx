...
function LoadingLabel({ loading, children }) {
  return <><span className={loading ? 'button-spinner' : 'button-spinner hidden'} aria-hidden="true" />{children}</>
}

function Toast({ toast, close, confirm, confirming }) {
  if (!toast) return null
  return <div className={`toast ${toast.type}`} role={toast.confirm ? 'alertdialog' : 'status'} aria-live="polite"><span>{toast.type === 'success' ? '✓' : '!'}</span><div><p>{toast.message}</p>{toast.confirm && <div className="toast-actions"><button type="button" onClick={close} disabled={confirming}>Cancel</button><button type="button" className="toast-confirm" onClick={confirm} disabled={confirming} aria-busy={confirming}><LoadingLabel loading={confirming}>{confirming ? 'Deleting…' : 'Delete'}</LoadingLabel></button></div>}</div>{!toast.confirm && <button aria-label="Dismiss message" onClick={close}>×</button>}</div>
}
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-card">
      ...
      <form onSubmit={submit} noValidate>
        ...
        {error && <p className="error">{error}</p>}<button className="primary wide" disabled={busy} aria-busy={busy}><LoadingLabel loading={busy}>{busy ? 'Please wait…' : mode === 'login' ? 'Sign in' : 'Create account'}</LoadingLabel></button>
      </form>
    </section>
  </main>
}
...
function App() {
  ...
  const [tasksLoading, setTasksLoading] = useState(false), [saving, setSaving] = useState(false)
  const [pendingAction, setPendingAction] = useState(''), [deleteCandidate, setDeleteCandidate] = useState(null)
  ...
  async function toggleTask(task) { setPendingAction(`toggle-${task.id}`); try { const done = task.status !== 'completed'; await api(`/tasks/${task.id}`, { method: 'PATCH', body: JSON.stringify({ status: done ? 'completed' : 'pending' }) }); await loadTasks(); notify(done ? 'Task marked as completed.' : 'Task moved back to pending.') } catch (err) { notify(err.message, 'error') } finally { setPendingAction('') } }
  function requestDelete(task) { setDeleteCandidate(task); setToast({ id: Date.now(), message: `Delete “${task.title}” permanently?`, type: 'warning', confirm: true }) }
  function cancelDelete() { if (pendingAction.startsWith('delete-')) return; setDeleteCandidate(null); setToast(null) }
  async function removeTask() { if (!deleteCandidate) return; const id = deleteCandidate.id; setPendingAction(`delete-${id}`); try { await api(`/tasks/${id}`, { method: 'DELETE' }); setDeleteCandidate(null); await loadTasks(); notify('Task deleted.') } catch (err) { notify(err.message, 'error') } finally { setPendingAction('') } }
  ...
  return <div className="app-shell">
    <Toast toast={toast} close={toast?.confirm ? cancelDelete : () => setToast(null)} confirm={removeTask} confirming={pendingAction.startsWith('delete-')} />
